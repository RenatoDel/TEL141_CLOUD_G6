...
"""
    ...
    def _build_user_data(self, vm_name: str, is_cirros: bool) -> str:
        if is_cirros:
...
"""
        ...
        for idx, iface in enumerate(interfaces):
            ...
            if iface.mac_address:
                lines.extend(
                    [
                        "    match:",
                        f'      macaddress: "{iface.mac_address}"',
                        f"    set-name: {guest_name}",
                    ]
                )
    ...
    def _create_seed_image(self, config: VMConfig, interfaces: list[VMInterface]) -> str:
        ...
        self._write_remote_text(user_data_path, self._build_user_data(config.name, is_cirros))
