...
async def upload_image_raw(
    ...
    original_filename = safe_filename(filename or "image.bin")
    stored_filename = original_filename
    stored_path = FILES_DIR / stored_filename
    part_path = FILES_DIR / f".{stored_filename}.part"

    with LOCK:
        data = load_catalog()
        ensure_unique_name_or_filename(data, name, original_filename)
        if stored_path.exists():
            raise HTTPException(status_code=409, detail="Ya existe un archivo físico con ese nombre")
    ...
    try:
        ...
        part_path.unlink(missing_ok=True)
        if final_created and stored_path.exists():
        ...
        if final_created and stored_path.exists():
            stored_path.unlink(missing_ok=True)
        raise HTTPException(status_code=500, detail=f"Error guardando imagen: {exc}") from exc
